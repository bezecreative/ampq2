...
3. **Re-encoded to WebP at 141KB (from 422KB, −67%)** — `images/hero.webp`, q80, 2400×1339; the phone-screen UI detail was checked at 1:1 and holds up clean. This closes the v8 audit's optional note that hero.jpg could re-encode smaller. **Per-pageview transfer drops ~281KB** (page total ~520KB → ~240KB). `images/hero.jpg` is kept as the same frame in optimized progressive JPEG (181KB) — swap the `src` back to that filename to revert formats instantly.
3a. **WebP is loaded via a plain `<img src>`, NOT a `<picture>` element.** A `<picture>` wrapper was tried and BROKE THE HERO: line ~229's `.hero > *:not(.hero-photo):not(.hero-scrim):not(.hero-top)` matched the new `<picture>` child and gave it `position: relative`, so the absolutely-positioned `.hero-photo` sized against the zero-height picture instead of `.hero` and the photo vanished. Do not reintroduce `<picture>` here without also amending that selector. WebP has universal support in current browsers, so the single-src approach needs no fallback markup and keeps the DOM byte-identical in structure.
...
6. **Hero framing reworked** (the phone was clipped at the right edge on the live site). `.hero-photo` is now `transform: scale(1.15) translateX(-9.3%)` (was `scale(1.38)` with no pan). Two things to know if this is ever retuned:
   - **Zoom and pan are paired.** The phone sits at ~73.5% across the photo; the pan holds it at ~74% across the frame as the zoom changes. To keep it put at another zoom, `pan% = (0.735 − 0.738/scale) × 100`: 1.38→20%, 1.30→16.7%, 1.20→12%, 1.15→9.3%, 1.10→6.4%, 1.00→0%. **`scale(1.0)` is the zoom-out floor** — the image exactly covers the box there, and below it the edges go empty. The phone is NOT the limiting factor.
   - **Order matters:** `translateX` must come AFTER `scale` so the shift lands in scaled space (a true 9.3%); before it, you get only ~8%. Horizontal `object-position` is a no-op here — the hero box is wider in aspect (2.03) than the 2400×1339 image (1.79), so `cover` crops vertically only and pan must come from the transform. `transform-origin: 0% 42%` unchanged.
   - **Desktop only.** Under 640px, line ~1319 sets `.hero-photo { transform: none; height: 262px; object-position: center 32% }` — the photo is its own band above a solid navy text area. Mobile framing is independent and unaffected by any zoom/pan change here.
7. Existing alt text still describes the new photo accurately ("A wash member reviewing their vehicles in the AMP app over coffee") and was left unchanged. No copy, layout, data, or motion changes. Verified: console clean, no overflow at 375/824/1440, em-dash count still 3.
