...
2. **New `api/rating.js`** — appends to the **`Email Rating`** tab of the same Google Sheet (shared `_google.js` auth). Row: `Timestamp | Rating | Subscriber ID | Campaign | User Agent`. Optional env `GOOGLE_RATINGS_TAB` overrides the tab name (defaults to `Email Rating`). No new Google setup — the existing service account already covers every tab.
