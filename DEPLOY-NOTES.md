...
6. **Hero pan corrected:** the phone was clipped at the right edge on the live site. `.hero-photo`'s transform is now `scale(1.38) translateX(-20%)` (was `scale(1.38)`), panning the image left 20% so the phone clears the frame. NOTE the function order — `translateX` AFTER `scale` applies the shift in scaled space for a true 20%; putting it before yields only ~14.5%. Horizontal `object-position` is a no-op on this element (the hero box is wider in aspect than the 2400×1339 image, so `cover` crops vertically only) — pan must come from the transform. `transform-origin: 0% 42%` is unchanged.
